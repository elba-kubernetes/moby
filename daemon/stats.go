...
func (w *BufferedWriter) Write(p []byte) (int, error) {
	...
	for srcRemaining + w.N > w.Size {
		copyOffset := srcStart - w.N
		for i := w.N; i < w.Size; i++ {
			w.Buffer[i] = p[i + copyOffset]
		}
	...
	destOffset := srcStart - w.N
	for i := srcStart; i < srcStart + srcRemaining; i++ {
		w.Buffer[i - destOffset] = p[i]
	}
	w.N += srcRemaining
